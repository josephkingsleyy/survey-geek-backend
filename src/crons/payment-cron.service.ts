...
export class PaymentCronService {
  ...
  async checkPendingPayments() {
    ...
    while (true) {
      ...
      this.logger.debug(
        `Processing batch of ${pending.length} payments, offset: ${skip}`,
      );
  ...
  async handlePaymentExpiration() {
    ...
    while (true) {
      ...
      this.logger.debug(
        `Processing expiration batch of ${payments.length} payments, offset: ${skip}`,
      );
    ...
    this.logger.log(
      `Payment expiration check for ${processedCount} payments completed.`,
    );
  }
}
